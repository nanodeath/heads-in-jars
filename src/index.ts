...
const agendaFileArg = process.argv.find((arg) =>
	arg.startsWith("--agenda-file="),
);
...
function readAgendaFile(filePath: string): AgendaFileData | null {
	try {
		...
		if (agenda.length === 0) {
			console.log(
				chalk.yellow(
					"Warning: No agenda items found in file. Using default agenda.",
				),
			);
			agenda.push(
...
function validateImports(): boolean {
	...
	console.log(
		"✓ utils.ts: createMessage, sleep, formatDuration, truncateText, containsAny, generateId",
	);
	...
	// Validate personas
	console.log(
		`✓ personas.ts: ${Object.keys(availablePersonas).length} personas available`,
	);
...
if (!isValidationMode) {
	console.log(
		chalk.cyan(figlet.textSync("AI Meeting", { horizontalLayout: "full" })),
	);
	console.log(
		chalk.cyan(figlet.textSync("Simulator", { horizontalLayout: "full" })),
	);
	console.log("\n");
}
	...
	try {
		...
		if (agendaFilePath) {
			...
			if (fileData) {
				...
				console.log(
					chalk.green(`Loaded ${agenda.length} agenda items from file:`),
				);
				...
				console.log(
					chalk.red(
						"Failed to load agenda from file. Falling back to manual input.",
					),
				);
		...
		if (agenda.length === 0) {
			...
			while (true) {
				const agendaItem = await new Input({
					...
					hint:
						itemNumber === 1
							? "(press Enter to submit, leave blank when finished)"
			...
			if (agenda.length === 0) {
				console.log(
					chalk.yellow("No agenda items provided. Using default agenda."),
				);
				agenda.push(
					"Project status updates",
					"Technical challenges",
		...
		await simulator.initialize(
			...
			async (
				...
				console.log(
					chalk.gray(
						"The AI has recommended some participants based on the meeting topic,",
					),
				);
				console.log(
					chalk.gray("but you can customize who will attend the meeting."),
				);
				...
				const selectedIds = await new MultiSelect({
					name: "personas",
					message:
						"Select meeting participants (space to toggle, enter to confirm):",
					choices,
		...
		console.log(
			chalk.green("│              Meeting Information                  │"),
		);
		console.log(
			chalk.green("╰───────────────────────────────────────────────────╯"),
		);
		...
		Object.values(simulator.agents)
			...
			.forEach((a) => {
				...
				console.log(
					chalk.white(`  • ${chalkColor(a.name)} - ${chalkColor(a.role)}`),
				);
			});
