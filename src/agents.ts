...
export class Agent {
    ...
    `;
		...
		try {
			...
			console.error(
				`Error generating introduction for ${this.name}:`,
				error.message,
			);
...
Based on this context, how urgently do you need to speak (1-5)?
    `.trim();
		...
		try {
			...
			const response = await withRetryLogic(
				...
				// Options
				{
					fallbackFn: async (error) => {
						console.error(
							`Using fallback for ${this.name} urgency calculation`,
						);
						return {
			...
			const messageBoost = Math.min(
				2.0,
				Math.max(0, 0.2 * this.messagesSinceLastSpoken),
			);
			...
			console.error(
				`Error calculating urgency for ${this.name}:`,
				error.message,
			);
    ...
    `;
		...
		try {
			...
			if (onStream) {
				...
				try {
							...
							if (fullResponse === "") {
								...
								for (const pattern of selfReferencePatterns) {
									if (chunk.startsWith(pattern)) {
										...
										debugLog(
											`Stripped self-reference prefix from response: "${pattern}"`,
										);
										break;
									}
								}
					...
					const approximateUsage = {
						input_tokens: formattedMessages.reduce(
					...
					debugLog(
						`💰 Approximate cost estimate for ${this.name} streaming response:`,
						{
							model: this.highEndModel,
							inputTokens: costEstimate.inputTokens,
							outputTokens: costEstimate.outputTokens,
					...
					console.error(
						`Error generating streaming response for ${this.name}:`,
						error.message,
					);
			...
			console.error(
				`Error generating response for ${this.name}:`,
				error.message,
			);
			return `[Error generating response: ${error.message}]`;
		}
	...
	printMessage(content: string, streaming = false, isFirstChunk = false): void {
		...
		const roleTitle =
			this.role || (this.agentId === "moderator" ? "Moderator" : this.agentId);
...
export class ModeratorAgent extends Agent {
	...
	constructor({
		...
		super({
			...
			persona:
				"Professional meeting facilitator who ensures discussions stay on track and all voices are heard",
			role: "Moderator",
			color: "whiteBright",
			client,
			lowEndModel,
	...
	async selectParticipants(): Promise<Record<string, any>> {
      ...
      ${JSON.stringify(
				Object.fromEntries(
					Object.entries(this.availablePersonas).map(([k, v]) => [
						k,
						v.description,
					]),
				),
    ...
    `;
		...
		try {
			...
			if (selectedIds.length < 2) {
				...
				const availableIds = Object.keys(this.availablePersonas).filter(
				...
				if (availableIds.length > 0) {
					for (
						let i = 0;
						i < Math.min(additionalNeeded, availableIds.length);
						i++
					) {
					...
					messages: [
						{
							role: "user",
							content:
								"Discussion transcript for the previous agenda item:\n" +
								currentItemMessages
									.map((m) => `${m.agentId || "User"}: ${m.content}`)
									.join("\n"),
						},
    ...
    `;
		...
		const recentMessages = conversation.slice(
			-Math.min(20, conversation.length),
		);
    ...
    `;
		...
		const recentMessages = conversation.slice(
			-Math.min(10, conversation.length),
		);
		...
		try {
			...
			if (Object.keys(availableParticipants).length === 0) {
				const agentIds = Object.keys(agents).filter(
					(id) => id !== "moderator" && id !== lastSpeakerId,
				);
			...
			if (nextSpeaker === lastSpeakerId) {
				...
				const eligibleAgents = Object.keys(agents).filter(
					(id) => id !== "moderator" && id !== lastSpeakerId,
				);
				nextSpeaker =
