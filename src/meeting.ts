...
export class MeetingSimulator {
   ...
   */
  async _initializeAgents(selectedPersonas: Record<string, PersonaInfo>): Promise<Record<string, Agent>> {
    ...
    while (meetingActive) {
      ...
      if (userTurn) {
        ...
        for (const agentId of participantAgentIds) {
          ...
          const agent = this.agents[agentId];
          const promise = agent.calculateUrgency(recentMessages, currentAgendaItem).then((urgency) => {
        ...
        if (global.isDebugMode) {
          ...
          for (const [agentId, score] of sortedScores) {
            ...
            const scoreColor = score >= 4 ? chalk.redBright : score >= 3 ? chalk.yellowBright : chalk.greenBright;
            console.log(chalk.gray(`${agent.name} [${agent.role}]: `) + scoreColor(`${score.toFixed(2)}`));
          }
