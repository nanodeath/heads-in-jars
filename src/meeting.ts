...
export class MeetingSimulator {
	...
	async initialize(
		...
		if (
			personaSelectionCallback &&
			typeof personaSelectionCallback === "function"
		) {
			updateStatus("Waiting for user to finalize participant selection...");
			finalSelectedPersonas = await personaSelectionCallback(
				recommendedPersonas,
				this.availablePersonas,
			);
		}
		...
		updateStatus(
			`Finalizing ${participantCount} participants for the meeting...`,
		);

		updateStatus(
			"Configuring participant personas and creating agent profiles...",
		);
		await sleep(300); // Small pause to show status
	...
	async _initializeAgents(
		...
		for (const [agentId, personaInfo] of Object.entries(selectedPersonas)) {
			...
			debugLog(
				`Setting up ${personaInfo.name} [${personaInfo.role}] (${currentPersona}/${personaCount})`,
			);
			...
			Object.entries(this.agents).forEach(([id, agent]) => {
				if (id !== agentId && id !== "moderator") {
					...
					if (global.isDebugMode) {
						debugLog(
							`Incremented message count for ${agent.name} to ${agent.messagesSinceLastSpoken}`,
						);
					}
				}
			});
	 ...
	 */
	_getRecentMessages(count = 10): Message[] {
	...
	async runMeeting(): Promise<void> {
		...
		while (meetingActive) {
			...
			if (userTurn) {
				...
				if (shouldMoveNext) {
					const nextItem = await this.moderator.nextAgendaItem(
						this.conversation,
					);
					...
					if (nextItem === null) {
						...
						// Generate the meeting conclusion
						const conclusionMessage = await this.moderator.endMeeting(
							this.conversation,
						);
				...
				const participantAgentIds = Object.keys(this.agents).filter(
					(id) => id !== "moderator",
				);
				...
				if (this.lastNonModeratorSpeaker) {
					debugLog(
						`Last non-moderator speaker: ${this.agents[this.lastNonModeratorSpeaker].name}`,
					);
				} else {
				...
				if (
					...
					debugLog(
						`Restricting ${this.agents[restrictedSpeakerId].name} from speaking consecutively`,
					);
				} else {
				...
				participantAgentIds.forEach((id) => {
					...
					thinkingStatus[id] =
						id === restrictedSpeakerId ? "zipped" : "thinking";
				});

				// Helper to format the status line
				const formatStatusLine = () => {
					return `Who's next: ${participantAgentIds
						.map((id) => {
				...
				for (const agentId of participantAgentIds) {
					...
					const agent = this.agents[agentId];
					const promise = agent
						.calculateUrgency(recentMessages, currentAgendaItem)
						.then((urgency) => {
				...
				if (Math.random() < 0.7 && eligibleSpeakers.length > 0) {
					...
					nextSpeaker = await this.moderator.chooseNextSpeaker(
						this.agents,
						this.conversation,
						restrictedSpeakerId,
					);
				...
				debugLog(
					`Selected next speaker: ${selectedAgent.name} (${speakerSelectionReason})`,
				);
				...
				if (global.isDebugMode) {
					...
					const sortedScores = Object.entries(urgencyScores).sort(
						(a, b) => b[1] - a[1],
					);
					...
					for (const [agentId, score] of sortedScores) {
						...
						console.log(
							chalk.gray(`${agent.name} [${agent.role}]: `) +
				...
				const responsePromise = agent.generateResponse(
					this.conversation,
					streamCallback,
				);
				...
				if (!interrupted) {
					...
					if (nextSpeaker !== "moderator") {
						...
						debugLog(
							`Updated last non-moderator speaker to: ${this.agents[nextSpeaker].name}`,
						);
					}
					...
					if (
						["exit", "quit", "end meeting"].includes(userInput.toLowerCase())
					) {
						console.log(chalk.cyan("\n=== Ending Meeting Early ===\n"));
						const conclusionSpinner = ora({
							text: "The moderator is preparing meeting summary...",
						...
						// Generate the meeting conclusion
						const endMessage = await this.moderator.endMeeting(
							this.conversation,
						);
	...
	async saveTranscript(filename: string): Promise<void> {
		...
		let summary = await this.moderator.generateMeetingSummary(
			this.conversation,
		);
