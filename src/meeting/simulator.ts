...
export class MeetingSimulator {
	...
	async initialize(
		...
		this.agents = await this._initializeAgents(
			finalSelectedPersonas,
			statusCallback,
		);
	...
	async introduceParticipants(): Promise<void> {
		...
		if (this.moderator) {
			...
			const spinnerColor =
				this.moderator.color.replace("Bright", "") || "white";
		...
		for (const [agentId, agent] of Object.entries(this.agents)) {
			if (agentId !== "moderator") {
				...
				try {
					...
					const systemPrompt = createIntroductionPrompt(
						agent.name,
						agent.persona,
					);
					...
					introSpinner.stop();
					console.error(
						`Error generating introduction for ${agent.name}:`,
						error.message,
					);
	...
	async runMeeting(): Promise<void> {
		...
		while (meetingActive) {
			...
			if (userTurn) {
				...
				const lastMessageWasUserInput =
					this.conversationManager.isLastMessageFromUser();
	...
	async generateAgentResponse(
		...
		if (!interrupted) {
			...
			if (agentId !== "moderator") {
				this.lastNonModeratorSpeaker = agentId;
				debugLog(
					`Updated last non-moderator speaker to: ${this.agents[agentId].name}`,
				);
			}
		} else {
	...
	async endMeetingEarly(): Promise<void> {
		...
		const conclusionSpinner = createSpinner(
			"The moderator is preparing meeting summary...",
			"cyan",
		).start();
