...
export async function selectNextSpeaker(
	...
	const participantAgentIds = Object.keys(agents).filter(
		(id) => id !== "moderator",
	);
	...
	if (!isLastMessageFromUser && lastSpeakerId) {
		...
		debugLog(
			`Restricting ${agents[restrictedSpeakerId].name} from speaking consecutively`,
		);
	} else {
	...
	for (const agentId of participantAgentIds) {
		...
		const promise = agent
			.calculateUrgency(conversation, currentAgendaItem)
			.then((urgency) => {
	...
	if (Math.random() < 0.7 && eligibleSpeakers.length > 0) {
		...
		nextSpeaker = await moderator.chooseNextSpeaker(
			agents,
			conversation,
			restrictedSpeakerId,
		);
	...
	debugLog(
		`Selected next speaker: ${agents[nextSpeaker].name} (${speakerSelectionReason})`,
	);
...
function logUrgencyScores(
	...
	for (const [agentId, score] of sortedScores) {
		...
		console.log(
			`\x1b[90m${agent.name} [${agent.role}]: \x1b[0m${scoreColor}${score.toFixed(2)}\x1b[0m`,
		);
	}
