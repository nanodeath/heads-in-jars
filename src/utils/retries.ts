...
export async function withRetryLogic<T>(
	...
	try {
		...
		const isEmptyResponse =
			error.message && error.message.includes("Empty response");

		if (
			(isRateLimitError || isGatewayError || isEmptyResponse) &&
			maxRetries > 0
		) {
			// Show retry message
			console.log(
			...
			if (options.fallbackFn) {
				console.log(
					`⚠️ Error occurred, using fallback for ${actionDescription}`,
				);
